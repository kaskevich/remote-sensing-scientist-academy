...
export const reviewedLessonDetails: Record<string, ReviewedLessonDetails> = {
  ...
  "lesson-06": {
    ...
    submissionChecklist: [
      "Functions have narrow names, explicit contracts, parameters, docstrings and return values",
      "Normal, boundary, missing and invalid cases have expected/actual evidence and assertions",
      "Boolean input is rejected before the general numeric check",
      "Three error categories are diagnosed from messages",
      "The independent function passes an explicit non-mutation check",
      "A ready, review or not-ready-for-pandas decision states the tested boundary",
      "Scientific validation is distinguished from software and regression testing",
    ],
    rubric: [
      { dimension: "Technical correctness", expectation: "Functions satisfy executable branch tests and preserve source records" },
      { dimension: "Conceptual understanding", expectation: "Explains contracts, parameters, returns, regression tests and traceback evidence" },
      { dimension: "Reproducibility", expectation: "Expected/actual results, assertions and failure diagnoses support each conclusion" },
      { dimension: "Scientific communication", expectation: "States tested representation scope and what passing tests cannot validate scientifically" },
    ],
    technicalMetadata: {
      ...sharedTechnicalMetadata,
      coreReferences: [
  ...
  "lesson-07": {
    ...
    submissionChecklist: [
      "The array contract records source, field, scope, order, shape, dtype and missingness",
      "Shape and positional mask alignment pass executable checks",
      "Vectorised results equal the Lesson 5 loop on the same known input",
      "Missing-value policy reports available and missing counts",
      "The two-dimensional bridge identifies rows, columns, shape, dimension and absent spatial meaning",
      "A justified ready, review or stop decision limits interpretation to each subset",
    ],
    rubric: [
      { dimension: "Technical correctness", expectation: "Array creation, summaries, missingness, masks and equivalence assertions return verified outputs" },
      { dimension: "Conceptual understanding", expectation: "Explains shape, dtype, vectorisation, positional alignment and array-versus-raster meaning" },
      { dimension: "Reproducibility", expectation: "Array contract, dataset DOI, field names, values, subset scope and checks are visible" },
      { dimension: "Scientific communication", expectation: "Reports denominators, missingness, selection and absent spatial metadata with summaries" },
    ],
    technicalMetadata: {
      ...sharedTechnicalMetadata,
      coreReferences: [
  ...
  "lesson-08": {
    ...
    submissionChecklist: [
      ...
      "Zenodo provenance, DOI, licence, file size, SHA-256 and parsing choices are recorded",
      "Shape, required fields and first identifiers pass executable expectations",
      "Identifiers, categories, dates, missing dates and duplicates are audited",
      "The schema note reconnects roles and unresolved definitions to the Lesson 2 contract",
      "A justified accept for quality audit, review or stop decision is recorded",
    ],
    rubric: [
      { dimension: "Technical correctness", expectation: "The documented path and parser load a table that satisfies structural and identifier assertions" },
      { dimension: "Conceptual understanding", expectation: "Explains byte identity, rows, columns, index, identifiers, dtypes and parsing boundaries" },
      { dimension: "Reproducibility", expectation: "Source, checksum, filename, path, versions, parsing and audit output are explicit" },
      { dimension: "Scientific communication", expectation: "Limits intake acceptance and does not infer unsupported units or field definitions" },
    ],
    technicalMetadata: {
      ...sharedTechnicalMetadata,
      coreReferences: [
  ...
  "lesson-09": {
    ...
    submissionChecklist: [
      "Quality profile separates completeness, conformance, consistency and identity",
      "The Lesson 6 classifier is upgraded with pd.isna and all six required assertions",
      "Coercion risk preserves and displays every unparseable present value",
      "AGB missingness is quantified overall and by sampling group with named denominators",
      "AGB and N_AGB_Sample missingness masks are compared, not inferred from equal totals",
      "Field-specific flags remain separate from source measurements",
      "Decision log records provenance, denominator, action, reversibility and unresolved risk",
      "Question-specific readiness does not describe not-flagged data as fully validated",
    ],
    rubric: [
      { dimension: "Technical correctness", expectation: "Profiles, scalar assertions, coercion masks, co-missingness and group coverage are accurate" },
      { dimension: "Conceptual understanding", expectation: "Separates quality dimensions and explains why data representations evolve a function contract" },
      { dimension: "Reproducibility", expectation: "Rules, denominators, flags, actions and reversibility are preserved in an audit trail" },
      { dimension: "Scientific communication", expectation: "Makes question-specific readiness decisions without inferring missingness cause or deleting uncertainty" },
    ],
    technicalMetadata: {
      ...sharedTechnicalMetadata,
      coreReferences: [
  ...
  "lesson-10": {
    ...
    submissionChecklist: [
      "The analysis specification names row unit, population, response, grouping, estimands and scope",
      "Filters are named, counted, inspected and traceable to Lesson 9 flags",
      "Summaries include distinct-ID n, centre and spread with correct meaning",
      "Grouped n and weighted means reconcile with the analysis population",
      "AGB coverage is reported separately from observed-value summaries",
      "Site-by-community counts expose composition and sparse combinations",
      "A justified descriptive-handover, review or stop decision avoids causal claims",
    ],
    rubric: [
      { dimension: "Technical correctness", expectation: "Filters, named aggregations and reconciliation checks return internally consistent values" },
      { dimension: "Conceptual understanding", expectation: "Explains population, row unit, denominators, centre, spread, weighting and composition" },
      { dimension: "Reproducibility", expectation: "Every summary traces to selected IDs, quality flags and missingness policy" },
      { dimension: "Scientific communication", expectation: "Uses cautious descriptive language and distinguishes variation, uncertainty and causality" },
    ],
    technicalMetadata: {
      ...sharedTechnicalMetadata,
      coreReferences: [
